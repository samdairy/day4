...
FROM payment;
--Example of a stored procedure
CREATE  PROCEDURE lateFe()
LANGUAGE plpgsql

      late_fee_amount DECIMAL(5,2)

     --add a late fee to customer peyment
     UPDATE payment
     SET amount = amount + late_fee_amount
        SELECT rental_id
        FROM rental
     ...
     --commit the above statement inside of our procedure transactomn
     COMMIT;
...
CREATE OR REPLACE FUNCTION update_platinum_member() RETURNS void AS $$
DECLARE 
